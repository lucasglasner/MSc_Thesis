...
# Basin DEM
dem_full = xr.open_dataset(
    "datos/topography/basins/"+cuenca+".nc", chunks="auto").Band1
dem = xr.open_dataset("datos/topography/basins/"+cuenca +
                      "_regridmodis.nc", chunks="auto").Band1
dem_era5 = xr.open_dataset(
    "datos/topography/basins/"+cuenca+"_ERA5LAND.nc", chunks="auto").z
dem_cr2met = xr.open_dataset(
    "datos/topography/basins/"+cuenca+"_CR2MET.nc", chunks="auto").Band1
dem_cortes = xr.open_dataset(
    "datos/topography/basins/"+cuenca+"_Cortes.nc", chunks="auto").Band1
...
aspect = xr.open_dataset(
    "datos/topography/basins/aspect/"+cuenca+".nc", chunks="auto").Band1

# slope

slope = xr.open_dataset("datos/topography/basins/slope/" +
                        cuenca+"_regridmodis.nc", chunks="auto").Band1
...
for i, height in enumerate(elevation_bands):
    ...
    mask4 = (~np.isnan(dem_era5.where(
        dem_era5 > height).where(dem_era5 < height+dz)))
    mask3 = (~np.isnan(dem_cr2met.where(
        dem_cr2met > height).where(dem_cr2met < height+dz)))
    mask2 = (~np.isnan(dem_cortes.where(
        dem_cortes > height).where(dem_cortes < height+dz)))
...
plt.savefig("plots/maipomanzano/datasetcomparison/terrainmodels.pdf",
...
for i, landclass in enumerate(land_uses.keys()):
    ...
    landcover_percent[i] = np.where(
        (landcover > ic) & (landcover < fc), 1, 0).sum()/6016322
...
for i, direct in enumerate(degrees.keys()):
    ...
    orientation_percent[i] = np.where(
        (aspect-45 > ic) & (aspect-45 < fc), 1, 0).sum()/463940
...
cax = fig.add_axes([ax1.get_position().xmin+0.02,
                    ...
                    0.02])
cb = fig.colorbar(azimuth, cax=cax, orientation="horizontal",
                  ticks=[0, 90, 180, 270])
...
slopes = axn.pcolormesh(slope.lon, slope.lat, slope,
                        transform=ccrs.PlateCarree(), cmap="bone", rasterized=True)
...
cb = fig.colorbar(slopes, cax=cax, orientation="horizontal",
...
plt.savefig("plots/maipomanzano/terrainstudy.pdf",
            dpi=150, bbox_inches="tight")
