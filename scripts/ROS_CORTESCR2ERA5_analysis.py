...
for yr in range(1984, 2016, 1):
    ...
    maxSWE.append(SWE)
mask = xr.where(xr.concat(maxSWE, 'max_years').mean(dim='max_years') > 30 ,1,0)
...
dSWEmax = dSWEmax.compute().where(mask)
...
dswe_vs_pr = dswe_vs_pr.reindex({'lon': ROS.lon, 'lat': ROS.lat},
                                method='nearest')
dswe_vs_pr = dswe_vs_pr.compute().where(dswe_vs_pr<1).where(mask)
...
ROS_rainratio = ROS_rainratio / \
    PR_CR2MET.where(PR_CR2MET > 10).compute().quantile(0.9, dim='time')
