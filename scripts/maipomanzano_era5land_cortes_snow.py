...
topo_cortes = xr.open_dataset("datos/topography/basins/RioMaipoEnElManzano_Cortes.nc").Band1
#BASIN shapefile
...
for i,height in enumerate(elevation_bands):
    ...
    mask3 = (~np.isnan(topo_cortes.where(topo_cortes>height).where(topo_cortes<height+dz)))
...
ax1.scatter(SCA_ianigla[mask1],SCA_cortes.reindex(SCA_ianigla.index)[mask1],alpha=0.3,s=5,color="tab:red",label="summer")
ax1.scatter(SCA_ianigla[mask2],SCA_cortes.reindex(SCA_ianigla.index)[mask2],alpha=0.3,s=5,color="tab:brown",label="autumn")
ax1.scatter(SCA_ianigla[mask3],SCA_cortes.reindex(SCA_ianigla.index)[mask3],alpha=0.3,s=5,color="tab:cyan",label="winter")
ax1.scatter(SCA_ianigla[mask4],SCA_cortes.reindex(SCA_ianigla.index)[mask4],alpha=0.3,s=5,color="tab:green",label="spring")
...
data2 = SCA_cortes.reindex(SCA_ianigla.index).groupby([SCA_ianigla.index.year,SCA_ianigla.index.dayofyear]).mean().unstack().T
...
# plt.savefig("plots/maipomanzano/cortes_SCA_study.pdf",dpi=150,bbox_inches="tight")
